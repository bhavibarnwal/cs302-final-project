...
def main():
    ...
    for gen in range(20):
        ...
        for iter in range(options.iters):
            if iter == 0:               # OFFSPRING 1
                scene = Scene()
                build_robot_skeleton(scene, seg1, 0.05, seg1*0.05*2.5, h1)
                scene.finalize()

                print(f"HEIGHT 1: {h1:.3f}, SEGMENT NUMBER: {seg1}")
            ...
            if iter == 5:               # OFFSPRING 2 
                scene = Scene()
                build_robot_skeleton(scene, seg2, 0.05, seg2*0.05*2.5, h2)
                scene.finalize()

                print(f"HEIGHT 2: {h2:.3f}, SEGMENT NUMBER: {seg2}")
            ...
            if iter == 10:              # OFFSPRING 3 
                scene = Scene()
                build_robot_skeleton(scene, seg3, 0.05, seg3*0.05*2.5, h3)
                scene.finalize()

                print(f"HEIGHT 3: {h3:.3f}, SEGMENT NUMBER: {seg3}")
            ...
            if iter == 15:              # OFFSPRING 4 
                scene = Scene()
                build_robot_skeleton(scene, seg4, 0.05, seg4*0.05*2.5, h4)
                scene.finalize()

                print(f"HEIGHT 4: {h4:.3f}, SEGMENT NUMBER: {seg4}")
            ...
            elif iter < 10 and iter > 4:     # OFFSPRING 2
                with ti.ad.Tape(loss):
                    forward()
                l = loss[None]
                losses.append(l)
            ...
            elif iter < 15 and iter > 9:    # OFFSPRING 3
                with ti.ad.Tape(loss):
                    forward()
                l = loss[None]
                losses.append(l)
        ...
        plt.plot(range(5), losses[:5], label=f"Height 1 = {h1}, {seg1} segments")
        plt.plot(range(5, 10), losses[5:10], label=f"Height 2 = {h2}, {seg2} segments")
        plt.plot(range(10, 15), losses[10:15], label=f"Height 3 = {h3}, {seg3} segments")
        plt.plot(range(15, 20), losses[15:20], label=f"Height 4 = {h4}, {seg4} segments")
        plt.legend()
        plt.show()
